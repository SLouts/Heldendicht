...
function FieldItem({
  ...
  const [state, formAction, pending] = useActionState(updateSiteRuleField, undefined);
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto flex gap-3 text-xs">
          ...
          <button
            type="button"
            disabled={index === 0}
            onClick={() => void moveSiteRuleField(field.id, "up")}
          ...
          <button
            ...
            onClick={() => void moveSiteRuleField(field.id, "down")}
            className="underline disabled:opacity-30"
          >
            下移
          </button>
          <button
            ...
            onClick={() => {
              if (confirm(`確定要刪除「${field.label}」這則規則嗎?`)) {
                void deleteSiteRuleField(field.id);
              }
            }}
...
export function SiteRuleFieldsEditor({ fields }: { fields: RuleFieldItem[] }) {
  ...
  return (
    <div className="mt-6">
      <div className="flex flex-col gap-2">
        {fields.map((field, i) => (
          <FieldItem key={field.id} field={field} index={i} total={fields.length} />
        ))}
        {fields.length === 0 && (
          <p className="text-sm text-muted-foreground">還沒有設定任何全站規則。</p>
        )}
      </div>
