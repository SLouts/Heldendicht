...
import {
  createNodeMediaUploadTicket,
  finalizeNodeMediaUpload,
  removeNodeMedia,
  type NodeMediaKind,
} from "@/lib/actions/nodeMedia";

const NODE_MEDIA_BUCKET = "node-media";

export function NodeMediaUpload({
  kind,
  label,
  nodeId,
  worldSlug,
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const ticket = await createNodeMediaUploadTicket(kind, nodeId, file.type, file.size);
      if ("error" in ticket) {
      ...
      const result = await finalizeNodeMediaUpload(kind, nodeId, worldSlug, nodeSlug, ticket.path);
          ...
          <button
            ...
            onClick={() => {
              if (confirm(`確定要移除${label}嗎?`)) {
                void removeNodeMedia(kind, nodeId, worldSlug, nodeSlug);
              }
            }}
